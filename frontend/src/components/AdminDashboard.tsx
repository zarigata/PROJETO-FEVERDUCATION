...
const AdminDashboard: React.FC = () => {
  ...
  if (window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
    return <DashboardLayout title={t('admin_dashboard')}><div className="p-8">{t('access_denied_local')}</div></DashboardLayout>;
  }
        ...
        <div className="mt-4">
          ...
          {activeTab === 'audit_logs' && (
            <section className="mb-6">
              ...
              <ul className="list-disc list-inside">
                {logs.map((log, i) => (<li key={i}>{JSON.stringify(log)}</li>))}
              </ul>
            </section>
          )}
